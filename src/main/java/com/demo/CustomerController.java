...
public class CustomerController {
    ...
    @PostMapping
    CustomerDtoResp createCustomer(@RequestBody CustomerDtoReqSub customer) {
        var cust = customerRepository.save(customer.toEntity());
        return new CustomerDtoResp(cust);
    }

    @GetMapping("/{id}")
    CustomerDtoResp getCustomerById(@PathVariable(value = "id") Long customerId) {
    ...
    @DeleteMapping("/{id}")
    void deleteCustomer(@PathVariable(value = "id") Long customerId) {
        var cust = customerRepository.findByIdF(customerId).getOrElseThrow(NotFoundException::new);
